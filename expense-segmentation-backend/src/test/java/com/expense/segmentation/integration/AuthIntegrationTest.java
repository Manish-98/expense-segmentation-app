...
class AuthIntegrationTest {
    ...
    void fullAuthFlow_RegisterLoginAndAccessProtectedEndpoint_ShouldSucceed() throws Exception {
        ...
        String tokenFromRegister = objectMapper.readTree(registerResponseBody)
                .get("token").asText();

        // Step 2: Access protected endpoint with token from registration
        mockMvc.perform(get("/auth/me")
                        .header("Authorization", "Bearer " + tokenFromRegister))
        ...
        String tokenFromLogin = objectMapper.readTree(loginResponseBody)
                .get("token").asText();
